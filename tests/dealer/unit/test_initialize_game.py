...
class TestInitializeGame:
    ...
    async def test_initialize_game_with_tool_context_but_missing_session_id(self):
        """Test initialize_game when tool_context provided but session_id is missing."""
        tool_context = Mock()
        tool_context.state = {}  # No session_id or user_id
        
        result = await initialize_game(tool_context=tool_context)
        # This should succeed since session_id is not required for initialization
        assert result["success"] is True
    
    async def test_initialize_game_with_tool_context_but_missing_user_id(self):
    ...
    async def test_initialize_game_with_tool_context_but_missing_both_ids(self):
